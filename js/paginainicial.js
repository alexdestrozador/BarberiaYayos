            ...
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    if (response.available) {
                        messageElement.innerHTML = 'La hora está disponible.';
                        messageElement.style.color = 'green';
                    } else {
                        messageElement.innerHTML = 'La hora ya está ocupada.';
                        messageElement.style.color = 'red';
                    }
                }
            }
        };
        xhr.send('adate=' + encodeURIComponent(adate) + '&atime=' + encodeURIComponent(atime));
    }
}
